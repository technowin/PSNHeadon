...
def menu_admin(request):
    ...
    try:
        ...
        Db.closeConnection()
        if request.method=="GET":
            return render(request,'Menu/menu_admin.html', {'entity':entity,'type':type,'name':name,'header':header,'data':data,'pre_url':pre_url})
...
def menu_master(request):
    ...
    try:
        ...
        Db.closeConnection()
        if request.method == "GET":
            return render(request, 'Menu/menu_master.html', {'menu': menu,'type': type,'roles': roles, 'users': users,'menu_id': menu_id,'order': 'order' if type == 'order' else None,'menus': menus if menu_id != '0' else None})
        elif request.method == "POST":
...
def menu_order(request):
    ...
    try:
        ...
        Db.closeConnection()
        if request.method=="GET":
            return render(request,'Menu/menu_master.html', {'type':type,'name':name,'header':header,'data':data})
        elif request.method=="POST":  
